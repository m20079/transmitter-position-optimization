...
from bayesian_optimization.bayesian_optimization import (
    double_transmitter_bayesian_optimization,
)
...
from conventional_method.random_search import (
    double_transmitter_random_search,
)
...
def double_transmitter_bo_simulation(
    ...
    for seed in range(simulation_number):
        ...
        train_indices_key_a, train_indices_key_b = random.split(
            key=train_indices_key, num=2
        )

        grid_indices = coordinate.create_grid_double_transmitter_indices(
        ...
        result: tuple[int, float, float, float, float, float] = (
            double_transmitter_bayesian_optimization(
                propagation=propagation,
                coordinate=coordinate,
        ...
        distance_error.append(result[3])
        data_rate_absolute_error.append(result[4])
        data_rate_relative_error.append(result[5])
        ...
        print(f"distance_error: {result[3]}", flush=True)
        print(f"data_rate_absolute_error: {result[4]}", flush=True)
        print(f"data_rate_relative_error: {result[5]}", flush=True)
...
def double_transmitter_rs_simulation(
    propagation: Propagation,
    coordinate: Coordinate,
    ...
    debug_name: str = f"double_transmitter_random_search_{transmitter_number}_{evaluation_function.__qualname__}"
    ...
    for seed in range(simulation_number):
        ...
        result: tuple[int, Array, Array, Array, Array, Array] = (
            double_transmitter_random_search(
                propagation=propagation,
                coordinate=coordinate,
                receivers_key=receivers_key,
        ...
        data_rate_absolute_error.append(float(result[4].block_until_ready()))
        data_rate_relative_error.append(float(result[5].block_until_ready()))
        ...
        print(
            f"data_rate_absolute_error: {float(result[4].block_until_ready())}",
            flush=True,
        )
        print(
            f"data_rate_relative_error: {float(result[5].block_until_ready())}",
            flush=True,
        )
