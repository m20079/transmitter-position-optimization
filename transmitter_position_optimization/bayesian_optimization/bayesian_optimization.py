...
def single_transmitter_optimization(
    ...
    max_search_number: Array = (coordinate.x_mesh + 1) * (coordinate.y_mesh + 1)
        ...
        if is_finished:
            ...
            final_x_position, final_y_position = (
                coordinate.convert_indices_to_transmitter_positions(
                    x_indices=x_train_indices[final_train_data_index],
                    y_indices=y_train_indices[final_train_data_index],
                )
            )
...
def double_transmitter_optimization(
    propagation: Propagation,
    coordinate: Coordinate,
    receivers: Receivers,
    frequency: float,
    ...
    max_search_number: Array = loop_number**2
    ...
    for count in range(x_train_indices_a.size, max_search_number):
        ...
        next_index, max_acquisition = jax.vmap(body_fun)(jnp.arange(0, loop_number))
        ...
        if is_finished:
            ...
            final_x_position_a, final_y_position_a = (
                coordinate.convert_indices_to_transmitter_positions(
                    x_indices=x_train_indices_a[final_train_data_index],
                    y_indices=y_train_indices_a[final_train_data_index],
                )
            )
            final_x_position_b, final_y_position_b = (
                coordinate.convert_indices_to_transmitter_positions(
                    x_indices=x_train_indices_b[final_train_data_index],
                    y_indices=y_train_indices_b[final_train_data_index],
                )
            )
...
def triple_transmitter_optimization(
    propagation: Propagation,
    coordinate: Coordinate,
    receivers: Receivers,
    frequency: float,
    ...
    loop_number: Array = (coordinate.x_mesh + 1) * (coordinate.y_mesh + 1)
    max_search_number: Array = loop_number**3
    ...
    for count in range(x_train_indices_a.size, max_search_number):
        ...
        def body_fun1(
            ...
            def body_fun2(i: Array) -> tuple[Array, Array]:
                ...
                input_x_test_data_c, input_y_test_data_c = (
                    coordinate.convert_indices_to_transmitter_positions(
                        x_indices=jnp.full(
                            (x_transmitter_positions.shape), j % (coordinate.x_mesh + 1)
                        ),
                        y_indices=jnp.full(
                            (y_transmitter_positions.shape),
        ...
        if is_finished:
            ...
            final_x_position_c, final_y_position_c = (
                coordinate.convert_indices_to_transmitter_positions(
                    x_indices=x_train_indices_c[final_train_data_index],
                    y_indices=y_train_indices_c[final_train_data_index],
                )
            )
