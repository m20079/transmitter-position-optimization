...
class BFGS(ParameterOptimization):
    def __init__(
        self: Self,
        count: int,
    ...
    @classmethod
    def tree_unflatten(cls, aux_data, children) -> "BFGS":
    ...
    def optimize(
        ...
        def body_fun(
            ...
            return (
                ...
                next_parameter,
                B
                - (B @ jnp.expand_dims(r, axis=1) @ jnp.expand_dims(r, axis=0) @ B)
                / (jnp.expand_dims(r, axis=0) @ B @ jnp.expand_dims(r, axis=1))
                + (jnp.expand_dims(s, axis=0) @ jnp.expand_dims(s, axis=1))
                / (jnp.expand_dims(r, axis=0) @ jnp.expand_dims(s, axis=1)),
            )
